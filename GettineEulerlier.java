...
public class GettineEulerlier {
    ...
    //longest part is probably just putting the numbers into a 2d array. afterwards I can run a double for loop to check each numbers left and rights and a diagnol
    public static int prodGriddy(){
        ...
        int[][] parse = new int[20][20];
        int[][] evaluated = new int[20][20];
        //turning the string into numbers
        for(int i =0 ; i<20 ; i++){
            for(int j =0 ; j<20 ; j++){
                parse[i][j] = Integer.parseInt(numbers.substring(0,2));
                if(numbers.length()> 3){
        ...
        //initial checks for: left to right,  up and down, and lastly a diagnal from left to right
        for(int i =0 ; i<16 ; i++){
            for(int j =0 ; j<16 ; j++){
                if(straight(i, j, parse) > down(i, j, parse) && straight(i, j, parse)>diag(i, j, parse)){
                    evaluated[i][j] = straight(i, j, parse);
                }
                else if(down(i, j, parse) > straight(i, j, parse) && down(i, j, parse)>diag(i, j, parse)){
                    evaluated[i][j] = down(i, j, parse);
                }
                else{
                    evaluated[i][j] = diag(i, j, parse);
                }
            }
        }
    ...
    public static int straight(int i, int j, int[][] numbers){
        return numbers[i][j]*numbers[i][j+1]*numbers[i][j+2]*numbers[i][j+3];
    }
    public static int down(int i, int j, int[][] numbers){
        return numbers[i][j]*numbers[i+1][j]*numbers[i+2][j]*numbers[i+3][j];
    }
    public static int diag(int i, int j, int[][] numbers){
        return numbers[i][j]*numbers[i+1][j+1]*numbers[i+2][j+2]*numbers[i+3][j+3];
    }
    public static int ODiag(int i, int j, int[][] numbers){
        return numbers[i][j]*numbers[i-1][j+1]*numbers[i-2][j+2]*numbers[i-3][j+3];
    }
